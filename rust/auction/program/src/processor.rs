...
pub struct AuctionDataExtended {
    ...
    /// last bidder token
    pub prev_bidder_token : Option<Pubkey>,
}
...
impl AuctionData {
    ...
    pub fn place_bid(
        ...
        self.bid_state.place_bid(bid, tick, minimum)
    }
    ...
    pub fn get_real_amount(
        ...
        if tick==0 {return Err(AuctionError::InvalidTickSize.into());}
        let reward = match reward_size {
            Some(val) => val,
            None => 0,
        };
        let start = match self.price_floor{
            PriceFloor::MinimumPrice(min) => min[0],
            _ =>0,
        };
            ...
            BidState::EnglishAuction { ref mut bids, max } => {
                match bids.last() {
                    Some(top) => {
                        ...
                        let mut minimum_bid : u128 = (bids[0].1 as u128)
                            .checked_add(tick as u128)
                            .ok_or(AuctionError::NumericalOverflowError)?;
                        if(bid.1 as u128 > minimum_bid){
                            bids.push(bid);
                            Ok(1)
                        }else {
                            Ok(0)
